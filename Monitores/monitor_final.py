...
class serialPlot:
    def __init__(self, serialPort='/dev/ttyACM0', serialBaud=9600, plotLength=100, dataNumBytes=2, numPlots=1):
        ...
        self.rawData = bytearray((numPlots + 1) * dataNumBytes)
        self.dataType = None
        if dataNumBytes == 2:
    ...
    def close(self,RecData):
        ...
        if RecData == 1:
                ...
                for packet in self.csvPackets:
                        ...
                        # tiempo
                        t_bytes = privateData[0:self.dataNumBytes]
                        t_value, = struct.unpack(self.dataType, t_bytes)

                        values = []

                        for i in range(self.numPlots):
                            index = i + 1
                            data = privateData[(index*self.dataNumBytes):(self.dataNumBytes + index*self.dataNumBytes)]
                            value, = struct.unpack(self.dataType, data)
                            values.append(value)

                        self.csvData.append([t_value, values[0], values[1], values[2]])
    ...
    def backgroundThread(self):    # retrieve data
        ...
        while (self.isRun):
            ...
            packetSize = len(self.rawData)

            while self.serialConnection.in_waiting >= packetSize:

                self.serialConnection.readinto(self.rawData)

                packet = copy.deepcopy(self.rawData)

                self.packetQueue.append(packet)   # para la grafica
                self.csvPackets.append(packet)    # para CSV
    ...
    def getSerialData(self, frame, lines, lineValueText, lineLabel, timeText,RecData):
        ...
        privateData = self.packetQueue[-1]   # tomar el paquete mas reciente
        self.packetQueue.clear()             # limpiar la cola
