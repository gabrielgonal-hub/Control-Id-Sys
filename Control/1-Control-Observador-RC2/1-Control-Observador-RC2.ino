...
void setup() {
  ...
  // K2 = p12*p22*(prod_a - 1/(p11*p22) - 1/(p21*p22) - 1/(p21*p32) - K1/(p11*p22) + 1/(p21*p22));
  // Kp = K1 + K2 + 1;
  K1 = p11*(sum_a - 1/p11 - 1/p21 - 1/p22 - 1/p32);
  K2 = p12*p22*(prod_a - 1/(p11*p22) - 1/(p11*p32) - 1/(p21*p22) - 1/(p21*p32) - K1/(p11*p22) - K1/(p11*p32) + 1/(p21*p22));
  Kp = K1 + K2 + 1 + (K1*R2+R1+R2)/R3;
  // Ganancias de Observador
  // H2 = sum_b - 1/p11 - 1/p21 - 1/p22;
  // H1 = p22*(prod_b - 1/(p11*p22) - 1/(p21*p22) - H2/p11 - H2/p21 + 1/(p21*p22));  
  H2 = sum_b - 1/p11 - 1/p21 - 1/p22 - 1/p32;
  H1 = p22*(prod_b - 1/(p11*p22) -1/(p11*p32) - 1/(p21*p22) - 1/(p21*p32) - H2/p11 - H2/p21 + 1/(p21*p22));
}
